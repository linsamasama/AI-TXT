...
import { Input, Button, Select, message, Card, Spin, Space, Typography, Progress, Tag, Tooltip } from "antd";
import { PlusOutlined, DeleteOutlined, OrderedListOutlined, DownloadOutlined, ReloadOutlined, ArrowRightOutlined, RightOutlined, FileTextOutlined } from "@ant-design/icons";
import { generateStoryStream, getModels } from "../api";
...
3. 只需要列出大纲。
注意：所有内容，包括简介、大纲和正文，要严格要求使用第一人称我来抒写。`,
  theme: ""
};
...
export default function StoryGenerator() {
  ...
  // 正文栏相关state（单独管理，单任务用于右栏显示）
  const [fullStoryContent, setFullStoryContent] = useState(""); // 正文内容
  const [fullStoryStatus, setFullStoryStatus] = useState("idle"); // idle/generating/completed/error
  const [fullStoryMessage, setFullStoryMessage] = useState(""); // 状态消息
  const [fullStoryProgress, setFullStoryProgress] = useState(0);
  const [fullStoryWordCount, setFullStoryWordCount] = useState(0);
  const [fullStoryTask, setFullStoryTask] = useState(null); // 当前正文栏正在生成的任务
  const fullStoryContentRef = useRef(null);
  const [fullStoryUsedTaskId, setFullStoryUsedTaskId] = useState(null);

  // 并发参数
  const maxConcurrency = 3;
  const maxFullConcurrency = 2; // 全文同时最大生成数
  let running = 0;
  let taskIndex = 0;
  let fullRunning = 0;
  let fullTaskIndex = 0;
            ...
            setTasks(prev => prev.map(task => {
              ...
              if (data.type === 'start') {
                return {
                  ...task,
                  status: 'generating',
                  statusMessage: data.message || "开始生成小说...",
                };
  ...
  const generateFullStoryTask = async (task) => {
    const fullInstruction = `请基于如下小说大纲内容，写出一篇完整小说正文，全篇严格以“我”为主角的第一人称叙述。\n\n【小说主题】：${task.theme}\n\n【大纲内容】：\n${task.outline}\n\n要求：\n- 内容生动感人、有起伏。\n- 结构完整、细节丰富。\n- 不要再重复输出大纲，直接进入正文。`;
    let currentContent = "";
    try {
      await generateStoryStream(
        {
        ...
        (data) => {
          if (data.type === 'content') {
            ...
            setTimeout(() => {
              const el = fullContentRefs.current[task.id];
  ...
  const handleGenerateFullStory = (task) => {
    ...
    if (!task || !task.content) {
      message.warning("需要选择一个已生成的任务");
      return;
    }
    const fullInstruction =
      `请基于如下小说大纲内容，写出一篇完整小说正文，全篇严格以“我”为主角的第一人称叙述。\n\n【小说主题】：${task.theme}\n\n【大纲内容】：\n${task.content}\n\n要求：\n- 内容生动感人、有起伏。\n- 结构完整、细节丰富。\n- 不要再重复输出大纲，直接进入正文。`;

    // 清理旧状态
    setFullStoryContent("");
    setFullStoryProgress(0);
    setFullStoryWordCount(0);
    setFullStoryMessage("正在连接AI服务...");
    setFullStoryStatus("generating");
    setFullStoryTask(task);
    setFullStoryUsedTaskId(task.id);
    ...
    generateStoryStream(
      {
        instruction: fullInstruction,
        model,
        wordCount: wordCount > 4000 ? wordCount : Math.max(4000, wordCount), // 自动提升字数
        existingContent: ""
      },
      (data) => {
        if (data.type === "content") {
          currentContent = data.fullContent || "";
          setFullStoryContent(currentContent);
          setFullStoryProgress(data.progress || 0);
          setFullStoryStatus("generating");
          setFullStoryWordCount(data.wordCount || 0);
          setFullStoryMessage(`正在生成中... (${data.wordCount || 0} 字)`);
          setTimeout(() => {
            if (fullStoryContentRef.current) {
              fullStoryContentRef.current.scrollTop = fullStoryContentRef.current.scrollHeight;
            }
          }, 80);
        } else if (data.type === "start") {
          setFullStoryStatus("generating");
          setFullStoryMessage(data.message || "开始生成正文...");
        } else if (data.type === "done") {
          setFullStoryContent(data.content || currentContent);
          setFullStoryStatus("completed");
          setFullStoryProgress(100);
          setFullStoryWordCount(data.wordCount || 0);
          setFullStoryMessage(`生成完成！（共 ${data.wordCount || currentContent.length} 字）`);
          setTimeout(() => {
            if (fullStoryContentRef.current) {
              fullStoryContentRef.current.scrollTop = fullStoryContentRef.current.scrollHeight;
            }
          ...
          setFullStoryStatus("error");
          setFullStoryMessage("生成失败：" + (data.error || "未知错误"));
        }
      }
    ).catch(e => {
      setFullStoryStatus("error");
      setFullStoryMessage("生成失败：" + (e.message || "未知错误"));
    });
  };
  ...
  const handleSaveTask = (task) => {
    ...
    a.href = url;
    const safeTheme = (task.theme || '未命名').replace(/[\\/:*?"<>|]/g, '_');
    ...
    message.success(`已保存：${safeTheme}.txt`);
  };
  ...
  const handleClear = () => {
    setInstructions([{
      id: Date.now(),
      basicInstruction: "严格以第一人我的视角，写一个温情小说，注意：正文里也严格要求使用第一人称我来抒写温情小说",
      theme: ""
    }]);
    setTasks([]);
    setFullTasks([]);
  ...
  const handleSaveFullStory = () => {
    ...
    const safeTheme = (fullStoryTask?.theme || '全文').replace(/[\\/:*?"<>|]/g, '_');
    const blob = new Blob([fullStoryContent], { type: "text/plain;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `${safeTheme}_全文.txt`;
  ...
  const handleClearFullStory = () => {
    ...
    setFullStoryWordCount(0);
    setFullStoryMessage("");
    setFullStoryStatus("idle");
    setFullStoryTask(null);
    setFullStoryUsedTaskId(null);
  };
  ...
  return (
    <div style={{ height: "calc(100vh - 120px)", display: "flex", flexDirection: "column" }}>
                ...
                {tasks.length > 0 && (
                  <div>
                    ...
                    <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                      ...
                      <Tag color="processing">生成中: {tasks.filter(t => t.status === 'generating').length}</Tag>
                      <Tag color="success">大纲完成: {tasks.filter(t => t.status === 'completed').length}</Tag>
                      <Tag color="error">失败: {tasks.filter(t => t.status === 'error').length}</Tag>
                    </div>
                    <div style={{ marginTop: 6, display: 'flex', gap: 12, flexWrap: "wrap" }}>
                      <Tag color="blue">已生成全文: {fullTasks.filter(t => t.status === 'completed').length}</Tag>
                      <Tag color="green">全文任务: {fullTasks.length}</Tag>
                      <Tag color="processing">
                        全文生成中: {fullTasks.filter(t => t.status === 'generating').length}
                      </Tag>
                      <Tag color="error">
              ...
              <Space direction="vertical" style={{ width: "100%" }} size="middle">
                {fullTasks.map((task, idx) => (
                  ...
                  >
                    ...
                    {task.content && (
                      <div
                        ref={(el) => { if (el) fullContentRefs.current[task.id] = el; }}
                        style={{
                          maxHeight: 280,
                          overflowY: "auto",
                          overflowX: "hidden",
      ...
      {/* 旧右栏单个正文保留，点小书按钮时弹出右栏渲染，兼容单全文 */}
      {fullStoryStatus !== "idle" && (
        <div
          style={{
            position: "fixed",
            top: 72,
            ...
            borderRadius: 10,
            boxShadow: "0 2px 18px -3px #bfbebe3a",
        ...
        >
          <Card
            ...
            bodyStyle={{ flex: 1, overflow: "auto", padding: 16 }}
            extra={
              <Button size="small" onClick={handleClearFullStory}>
                关闭
              </Button>
            }
          >
            <div style={{fontWeight: 500, marginBottom: 8, fontSize: 15}}>
              主题：{fullStoryTask?.theme || "--"}
            </div>
            {fullStoryStatus === "generating" && (
              <div style={{ marginBottom: 8, fontSize: 12, color: "#666" }}>
                <Spin size="small" style={{ marginRight: 6 }} />
                {fullStoryMessage}
                <Progress
                  percent={fullStoryProgress}
                  size="small"
                  status="active"
                  style={{ marginTop: 6 }}
                />
            ...
            {fullStoryStatus === "error" && (
              <div style={{ color: "#ff4d4f", marginTop: 12 }}>{fullStoryMessage}</div>
            )}
          </Card>
        </div>
      )}
