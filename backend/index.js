...
const unifiedGenerator = new UnifiedContextGenerator();

// 引入简化生成器
const { addSimpleAPI } = require('./simple_generator');

// 统一上下文生成 API（SSE）
app.post('/story/generate-unified-context', async (req, res) => {
  const {
    theme,
    instruction,
  ...
  if (!theme || typeof theme !== 'string' || !theme.trim()) {
    return res.status(400).json({ error: '缺少必要参数：theme（不能为空）' });
  }
  if (!targetWordCount || typeof targetWordCount !== 'number' || targetWordCount <= 0) {
    return res.status(400).json({ error: 'targetWordCount 必须是正数' });
  }
  ...
  const serverTimeout = setTimeout(() => {
    res.write(`data: ${JSON.stringify({ type: 'error', error: 'Server timeout - please retry' })}\n\n`);
    res.end();
  }, 10 * 60 * 1000);
  ...
  try {
    const generator = unifiedGenerator.generateUnifiedContext(
      { theme, instruction, targetWordCount, model, options },
      res
    );
    ...
    res.write(`data: ${JSON.stringify({
      type: 'error',
      error: errorDetail,
      canRetry: unifiedGenerator.canRetry(error)
    })}\n\n`);
    res.end();
  }
});
...
// 获取生成进度 API
app.get('/story/generation-progress/:cacheId', async (req, res) => {
  ...
  try {
    const progress = await unifiedGenerator.progressCache.loadProgress(cacheId);
    ...
    res.json({ 
      success: true, 
      progress,
      cacheStats: unifiedGenerator.progressCache.getCacheStats()
    });
  } catch (error) {
    console.error('获取生成进度失败:', error);
    res.status(500).json({ error: 'Failed to load progress', detail: error.message });
  }
});

// 恢复中断的生成 API
app.post('/story/resume-generation', async (req, res) => {
  ...
  try {
    ...
    // 执行恢复生成
    const generator = unifiedGenerator.generateUnifiedContext(resumeParams, res);
...
// 缓存管理 API
app.get('/story/cache-stats', (req, res) => {
  try {
    const stats = unifiedGenerator.progressCache.getCacheStats();
    const cacheList = unifiedGenerator.progressCache.listAllCache();
    
    res.json({
      success: true,
      stats,
      cacheList: cacheList.slice(0, 20) // 返回最近 20 个
    });
  } catch (error) {
...
// 清理缓存 API
app.delete('/story/cache/:cacheId', async (req, res) => {
  const { cacheId } = req.params;
  
  try {
    await unifiedGenerator.progressCache.deleteCache(cacheId);
